...
def get_action():
    ...
    global FIND_MODE, start_distance, yolo_results, turret_evaluate, real_enemy_pos, lidar_rotation
...
def get_info():
    global last_bullet_info, turret_evaluate, time, lidar_data, FIND_MODE, enemy_pos, real_enemy_pos, lidar_rotation
    ...
    if last_bullet_info:
        ...
        if last_bullet_info.get("hit") == "terrain":
            ...
            FIND_MODE = True
            control = "reset"
            # 'is_hit' 값을 기존 dict에 추가
            if turret_evaluate and isinstance(turret_evaluate[0], dict):
                turret_evaluate[0]['is_hit'] = 0
                # 모든 key가 다 들어있을 경우에만 저장
                expected_keys = {"distance_to_enemy", "detect_real_diff", "is_hit"}
                if expected_keys.issubset(turret_evaluate[0].keys()):
                    df = pd.DataFrame(turret_evaluate)
                    df.to_csv("lidar_detection_is_hit.csv", mode='a', header=not os.path.exists("lidar_detection_is_hit.csv"), index=False)             
                # 이후 초기화
                last_bullet_info = {}
                enemy_pos = {}
                turret_evaluate.clear()
