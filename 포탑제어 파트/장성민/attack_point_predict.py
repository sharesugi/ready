...
def info():
    global last_bullet_info, turret_info, saved_data
    ...
    pos_pitch = data.get("playerBodyY", 0) # 전차의 앞뒤 기울기 (Pitch)
    pos_yaw = data.get("playerBodyZ", 0)  # 전차의 좌우 기울기 (Roll)  
    ...
    if impact_point:
        print(f" 예상 타격 지점: x={impact_point['x']:.2f}, y={impact_point['y']:.2f}, z={impact_point['z']:.2f}")
        print(f" 전차 기울기: pitch={pos_pitch:.2f}°, yaw={pos_yaw:.2f}°")
        ...
        if last_bullet_info:
            ...
            turret_info = [
                impact_point['x'], impact_point['y'], impact_point['z'],  # 예측 타격
                bullet_x, bullet_y, bullet_z,                              # 실제 타격
                pos_pitch, pos_yaw, yaw_deg, pitch_deg                    # 전차/터렛 정보
            ]
            print(f"💥 실제 타격 지점: x={bullet_x:.2f}, y={bullet_y:.2f}, z={bullet_z:.2f}")
            if len(turret_info) >= 10:
                saved_data.append(turret_info[:])
                df = pd.DataFrame(saved_data, columns=["predicted_hit_x", "predicted_hit_y", "predicted_hit_z",
                                                       "actual_hit_x", "actual_hit_y", "actual_hit_z",
                                                       "pos_pitch", "pos_yaw", "turret_yaw", "turret_pitch"])
                df["errors"] = np.sqrt(
                    (df["predicted_hit_x"] - df["actual_hit_x"])**2 +
                    (df["predicted_hit_y"] - df["actual_hit_y"])**2 +
                    (df["predicted_hit_z"] - df["actual_hit_z"])**2
                )                                    
                df.to_csv("attack_predict_vs_actual.csv", index=False)
            last_bullet_info = {}
            turret_info = []
        else:
            last_bullet_info = {}
