...
def detect():
    global lidar_data, enemy_pos, FIND_MODE, yolo_results, body
...
def get_action():
    ...
    global FIND_MODE, start_distance, yolo_results, dist_df
    ...
    if FIND_MODE: # 적 전차를 탐색하는 상태일 때
        ...
        if start_distance >= 80 or start_distance <= 20:
        ...
        if not yolo_results: # 전차 인식은 됐는데 그에 해당하는 라이다 포인트가 없다면 정지
            ...
            if aim_ready:
                dist_df = distance - start_distance
...
enemy_pos = {} # 적 전차의 위치
turret_evaluate = [] # 평가를 위해서 사용했던 변수
time = 0 # 시뮬레이터 시간
body = {}
...
def get_info():
    ...
    body = {'x': body_x, 'y': body_y, 'z': body_z}
