...
def detect_obstacle_and_hill(df):
    ...
    for i in df['line_group'].unique():
        ...
        if len(coords) <= 5:   # 값이 2개만 들어오면 데이터 부족 처리. map 에 반영 안 하도록
            print("data 부족")  # 나중에 함수 넣을 때 return 처리하거나 해당 값은 지우면 됨... 
            print()
            continue  
        ...
        # and 각도가 느슨하게 꺾이는 것이 3번 이상 발생하면 언덕...
            ...
            # 이 부분 추후 수정 필요...
            print(f"판단 안됨_sum_angle: {sum_angle}, sharp_turns: {sharp_turns}, loose_turns: {loose_turns}")
        print()
...
def info():
    ...
    lidar_data = [  
        ...
        if pt.get("verticalAngle", 0) <= 2.045454 and pt.get("isDetected", False) == True
    ]
    if not lidar_data:
