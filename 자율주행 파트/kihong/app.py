...
import fire, drive
import matplotlib # A* path 실시간 시각화
matplotlib.use('Agg')  # GUI 없는 서버에서도 작동하게 함 # A* path 실시간 시각화
import matplotlib.pyplot as plt # A* path 실시간 시각화
import matplotlib.image as mpimg # A* path 실시간 시각화
from matplotlib.lines import Line2D # A* path 실시간 시각화
import io # A* path 실시간 시각화
...
def get_path_image():
    ...
    # 배경 이미지 로드
    try:
        background = mpimg.imread("map_capture_0624.png")
    except FileNotFoundError:
        return "❌ map_capture_0624.png 파일을 찾을 수 없습니다.", 500
...
def real_time_map():
    ...
    <body>
        ...
        <div id="map-container">
            <img src="/static/map_capture_0624.png" id="map-bg">
    ...
    """
# setInterval(fetchAndDraw, 1000) 이 부분에 따라서 원하는 시간(1000=1초)마다 새로고침 됨.
# http://localhost:5005/a_star_path_real_time 에서 볼 수 있음.
